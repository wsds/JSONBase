...
enum {
	FRAME_PARSE_STATE_HDR1, FRAME_PARSE_STATE_HDR2, FRAME_PARSE_STATE_HDREX, FRAME_PARSE_STATE_MASKEY, FRAME_PARSE_STATE_DATA, FRAME_PARSE_STATE_CLOSED, FRAME_PARSE_STATE_ERROR,
};
...
int ws_encode(ws_frame_type_t frame_type, const uint8_t *input, uint32_t input_len, uint8_t **output, uint32_t *output_len) {
	...
	*output_len = 0;
	...
	if (input_len <= 125) { //0 byte
		second_byte = (uint8_t) input_len;
	} else if (input_len <= 0xffff) { //2 bytes
		header_len += 2;
		second_byte = 126;
	} else { //8 bytes
		header_len += 8;
	...
	memcpy((void *)(buf + header_len), (void *)input, input_len);
	*output = buf;
	...
	while ((*pcur_pos) < input_len) {
		...
		case FRAME_PARSE_STATE_HDREX:
			if (126 == UB2.PayloadLen) {
				...
				if (input_len - (*pcur_pos) + ctx->recv_len >= expect_len) {
					...
					ctx->recv_len = 0;
					if (ctx->frame_hdr.xpl.xpl16 < 126) { // invalid ex payload length
						ctx->status = FRAME_PARSE_STATE_ERROR;
				...
				if (input_len - (*pcur_pos) + ctx->recv_len >= expect_len) {
					...
					ctx->recv_len = 0;
					if ((ctx->frame_hdr.xpl.xpl64 >> 63) != 0) { // invalid ex payload length
						ctx->status = FRAME_PARSE_STATE_ERROR;
						return WS_ERROR_INVALID_LENGTH;
					}
					ctx->frame_hdr.data_len = (uint32_t) ctx->frame_hdr.xpl.xpl64;
					if (ctx->frame_hdr.data_len > WS_MAX_FRAME_DATA_LENGTH) { // invalid ex payload length
						ctx->status = FRAME_PARSE_STATE_ERROR;
						return WS_ERROR_INVALID_LENGTH;
					}
					ctx->status = FRAME_PARSE_STATE_MASKEY;
		...
		case FRAME_PARSE_STATE_MASKEY:
			if (UB2.Mask) {
				...
				if (input_len - (*pcur_pos) + ctx->recv_len >= expect_len) {
					memcpy((void *)(ctx->frame_hdr.maskey + ctx->recv_len), (void *)(input + (*pcur_pos)), expect_len - ctx->recv_len);
					(*pcur_pos) += expect_len - ctx->recv_len;
					ctx->recv_len = 0;
					ctx->recv_buf = NULL;
				} else {
					memcpy((void *)(ctx->frame_hdr.maskey + ctx->recv_len), (void *)(input + (*pcur_pos)), input_len - (*pcur_pos));
		...
		case FRAME_PARSE_STATE_DATA:
			...
			if (NULL == ctx->recv_buf) {
				ctx->recv_buf = (uint8_t*)malloc(ctx->frame_hdr.data_len + 1);
			}
			if (input_len - (*pcur_pos) + ctx->recv_len >= ctx->frame_hdr.data_len) {
				memcpy((void *)(ctx->recv_buf + ctx->recv_len), (void *)(input + (*pcur_pos)), ctx->frame_hdr.data_len - ctx->recv_len);
				...
				*output_len = ctx->recv_len;
				...
				memset((void *)&ctx->frame_hdr, 0, sizeof(ctx->frame_hdr));
				ctx->status = FRAME_PARSE_STATE_HDR1;
				ctx->recv_len = 0;
				ctx->recv_buf = NULL;
				return WS_ERROR_NOERR;
			} else {
				memcpy((void *)(ctx->recv_buf + ctx->recv_len), (void *)(input + (*pcur_pos)), input_len - (*pcur_pos));
				ctx->recv_len += input_len - (*pcur_pos);
				(*pcur_pos) = input_len;
				return WS_ERROR_NEED_MORE_DATA;
			}
...
static int handle_data_mask(ws_ctx_t* ctx) {
	if (0 == UB2.Mask)
		return WS_ERROR_NOERR;
	if (NULL == ctx->recv_buf || 0 == ctx->recv_len)
		return WS_ERROR_INVALID_FRAME;
	uint32_t i = 0;
	for (i = 0; i < ctx->recv_len; ++i) {
