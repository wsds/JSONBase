...
DMString * newDMString(int length) {
	DMString * dm_string1 = (DMString *) map_malloc(length);
	dm_string1->used_length = 0;
	dm_string1->type = TYPE_STRING;
	if (length == 0) {
		dm_string1->isZero = IS_ZERO;
	}
	return dm_string1;
}

void freeDMString(DMString * dm_string) {
	if (dm_string->isZero == IS_ZERO) {
		dm_string->max_length = 0;
	}
	map_free((MemorySpace *) dm_string);
}
...
DMString& DMString::operator%(DMString * dm_string) {
	this->format(dm_string->char_string, dm_string->used_length);
	return *this;
}
