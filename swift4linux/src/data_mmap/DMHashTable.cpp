...
DMHashTable * newDMHashTable() {
	MemorySpace * space = map_malloc(sizeof(DMHashTable));
...
DMKeyValue * newDMKeyValue() {
	MemorySpace * space = map_malloc(sizeof(DMKeyValue));
...
MemorySpace* DMHashTable::get(MemorySpace* key) {
	...
	unsigned int hash = dictGenHashFunction1(key->pointer, key->length);
...
bool DMHashTable::resize() {
	...
	this->entry_map = map_malloc(mem_size);
	MemorySpace ** elements = (MemorySpace **) this->entry_map->pointer;
